...
def main():
  ...
  while str_Ficar == "S": 
    ...
    if str_Comando != "VF": input_VF = float(input("Digite o valor do Valor Futuro(VF) em R$: "))
    if str_Comando != "CI": input_CI = float(input("Digite o valor do Capital Inicial(CI) em R$: "))
    if str_Comando != "T": input_T = int(input("Digite a quantidade de Periodos(T): "))
    if str_Comando != "I": input_I = float(input("Digite o valor da Taxa de juros(I) dividido por 100: "))
    if str_Comando != "PGTO": input_PGTO = float(input("Digite o valor do Pagamento Mensal em R$: "))

    match str_Comando:
      case "VF": fun_CalcularVF(input_CI,input_I,input_T,input_PGTO)
      case "CI": fun_CalcularCI(input_VF,input_I,input_T, input_PGTO)
      case "T": fun_CalcularT(input_VF,input_CI,input_I, input_PGTO)
      case "I": fun_CalcularI(input_VF,input_CI,input_T)
      case "PGTO": fun_CalcularPGTO(input_VF,input_CI,input_T, input_I)
